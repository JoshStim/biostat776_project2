...
By taking a glimpse at the merged dataset, we are able to confirm that it in fact has 83964 observations and 13 columns.
...
df %>%
  ...
  geom_line() +
  labs(
    title = "Daily Temperature Extremes in Australia (from 2014 - 2019)",
    subtitle = "Maximum and minimum temperatures vary by season and city",
    x = "Date (ymd)",
    y = "Temperature (C)",
...
plot_rainfall_hist <- function(cityname, yr) {
  ...
  assertthat::assert_that(
    ...
    msg = "`city_name` or `year` was not found in `df`.")
...
```{r, warning = FALSE}
plot_rainfall_hist("BRISBANE", 1991)
```
